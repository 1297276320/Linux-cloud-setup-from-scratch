...
## （三）设置为自启
为避免frp因异常情况崩溃后导致ssh无法连接，需修改配置文件，使服务每次断开后自启。frp在0.59版本会有特殊的显示成功连接，但实际已经断连的情况，所以采用**crontab**系统级定时输入重启代码。
### 1. systemd介绍
systemd介绍：
...
- **but**:0.59版本可能会出现随机断连的问题。通过创建frpc的系统自启文件，和系统级crontab组件，避免断连。
...
2. 加入这一行：
```bash
0 3 */2 * * /usr/bin/systemctl restart frpc
```
| 字段                              | 意义                |
| ------------------------------- | ----------------- |
| 0 3                             | 凌晨 3:00           |
| */2                             | 每 2 天             |
| * *                             | 每月、每星期            |
| /usr/bin/systemctl restart frpc | 通过 systemd 重启服务 |
...
**背景说明**  
Swap（交换空间）最初是为了在物理内存不足时，将部分不活跃的内存数据交换到磁盘上，从而避免系统因内存耗尽而崩溃。然而，随着物理内存价格下降，以及磁盘 I/O 速度远低于内存访问速度，频繁使用 Swap 反而可能导致系统整体性能下降。目前更推荐的做法是增加物理内存，并合理配置 Swap 使用策略，避免不必要的内存交换。

**问题描述**  
在本项目使用的 R730 服务器中，系统默认设置了 2GB 的 Swap 空间，并将 `swappiness` 参数设为 60。这意味着当物理内存使用率接近 **40%(1-60%=40%)** 时，系统就开始将内存数据交换到磁盘，这在多数生产环境中是不合理的，容易引发性能瓶颈。

**优化方案**  
为解决该问题，建议执行以下两个操作之一或同时进行：  
1. 完全禁用 Swap（适用于内存充足且确定无需交换的场景）；  
2. 大幅降低 Swap 使用倾向，减少不必要的磁盘交换。


### 1. 禁用 Swap

运行以下命令立即关闭所有 Swap 空间：
...
**注意事项**：  
执行此命令后，系统会将 Swap 中的数据逐步移回物理内存。由于磁盘 I/O 较慢，该过程可能需要一定时间。通过 `htop` 等工具监控 Swap 与内存使用情况，直至 Swap 使用量降为 0。

---

### 2. 调整 Swap 使用倾向（swappiness）

通过修改系统参数 `vm.swappiness`，可控制系统使用 Swap 的倾向程度。该值范围为 0–100，数值越低，系统越倾向于保留在物理内存中。

**操作步骤**：

1. 编辑配置文件 `/etc/sysctl.conf`，在文件末尾**添加**或修改以下行：
   ```
   vm.swappiness=1
   ```
...
3. 验证当前生效的 swappiness 值：
   ```bash
   cat /proc/sys/vm/swappiness
   ```

**建议值说明**：  
- `vm.swappiness=1`：仅在内存极度紧张时使用 Swap，适用于大多数服务器场景。  
- 若设为 `0`，则系统不会主动触发 Swap，但仍可能在内存在严重不足时使用。

---
**最终建议**：  
若服务器物理内存充足，且无特殊需求，建议直接禁用 Swap；若希望保留 Swap 作为应急备用，则可将 `swappiness` 调整为 1–10 之间的低值。